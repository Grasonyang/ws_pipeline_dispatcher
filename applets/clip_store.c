...
int main(int argc, char *argv[])
{
    ...
    if (get_key != NULL) {
        ...
        while (getline(&line, &cap, stdin) > 0) {
            char *session = pipeline_json_find_string_value(line, "session_id");
            char *ts = pipeline_json_find_scalar_value(line, "ts");
            char *path = pipeline_json_find_string_value(line, "path");
